...
1. **数据文件修改**: 大部分内容修改通过更新 `src/data/` 下的配置文件即可
2. **新增页面**: Astro 使用文件系统路由，新增 `.astro` 文件即创建新页面
3. **类型安全**: 项目使用 TypeScript 严格模式，注意类型提示
4. **图片资源**: 封面图放在 `public/assets/` 下，路径以 `/assets/` 开头
5. **SEO 配置**: 每个页面的 TDK 在 `content.ts` 中对应配置
